...
bool xmlConfigLoader::loadCommon()
{
	...
	common->QueryIntAttribute("uiType", (int*)&_uiConfig->_layersUIType);
	...
	common->QueryAttribute("FXAA", &_appConfig->_FXAA);
...
bool xmlConfigLoader::saveCommon()
{
	...
	if (root)
	{
		...
		if (common)
		{
			...
			common->SetAttribute("uiType", (int)_uiConfig->_layersUIType);
			...
			common->SetAttribute("FXAA", _appConfig->_FXAA);
