...
bool ImageButton(const char* id, const sf::Texture& texture, const sf::Color& bgColor,
                 const sf::Color& tintColor) {
    return ImageButton(id, texture, static_cast<sf::Vector2f>(texture.getSize()),
                       bgColor,
                       tintColor);
}
...
bool ImageButton(const char* id, const sf::RenderTexture& texture,
                 ...
                 const sf::Color& tintColor) {
    return ImageButton(id, texture, static_cast<sf::Vector2f>(texture.getSize()), bgColor,
                       tintColor);
}
...
bool ImageButton(const char* id, const sf::Sprite& sprite,
    ...
    return ImageButton(id, sprite, sf::Vector2f(spriteSize.width, spriteSize.height),
                       bgColor, tintColor);
}
...
void DrawRect(const sf::FloatRect& rect, const sf::Color& color, float rounding,
              ImDrawFlags rounding_corners, float thickness) {
    ImDrawList* draw_list = ImGui::GetWindowDrawList();
